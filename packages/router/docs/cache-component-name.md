...
`vue-smart-router` 与对应版本的官方 `vue-router` 保持相同的 peer 要求，需与项目中的 **Vue 3** 版本匹配（参见包内 `peerDependencies`）。从 `vue-router` 迁到本 fork 时，请**全局**把依赖与 import 从 `vue-router` 改为 `vue-smart-router`（子路径如 `vue-smart-router/auto-routes` 同理）。
