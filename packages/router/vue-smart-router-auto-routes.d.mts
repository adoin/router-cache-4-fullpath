...
// `routes.d.ts` and wired into `vue-smart-router`'s `TypesConfig` from there, so the
// typed routes work even if this module is never imported (e.g. when only
// `vue-smart-router/auto-resolver` is used). See vuejs/router#2696.
// TODO(v6): rename to `RouteMap` and host the interface directly on `vue-smart-router`.
