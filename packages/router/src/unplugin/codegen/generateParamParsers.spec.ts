...
const parsersTest = test.extend<{ parsersDir: string }>({
  parsersDir: async ({}, use) => {
    const dir = mkdtempSync(join(tmpdir(), 'vue-smart-router-param-parsers-'))
...
describe('isRawParamParserSource', () => {
  ...
  it('returns true when parser is defined via defineParamParserRaw', () => {
    const source = `
      import { defineParamParserRaw } from 'vue-smart-router/experimental'
  ...
  it('supports aliased imports of defineParamParserRaw', () => {
    const source = `
      import { defineParamParserRaw as defineRaw } from 'vue-smart-router/experimental'
  ...
  it('returns false when the parser export uses a different identifier', () => {
    const source = `
      import { defineParamParserRaw, defineParamParser } from 'vue-smart-router/experimental'
      export const parser = defineParamParser({
        get: v => v,
        set: v => String(v),
      })
