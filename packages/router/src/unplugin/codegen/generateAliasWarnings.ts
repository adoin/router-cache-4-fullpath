...
export function generateAliasWarnings(tree: PrefixTree): string {
  ...
  for (const node of tree.getChildrenDeepSorted()) {
    for (const alias of node.value.alias) {
      if (!alias.startsWith('/')) {
        warnings.push(
          `console.warn('[vue-smart-router] Alias "${alias}" for route "${node.value.fullPath}" must be absolute (start with "/"). Relative aliases are not supported in file-based routing.')`
        )
      }
    }
  }
