...
`@vue/devtools-api` 仅在使用 Vue Router 的开发构建版本时需要。当使用生产构建版本 `vue-smart-router.esm-browser.prod.js` 时可以移除。
...
对应的 Vue Router 生产构建版本为 `vue-smart-router.global.prod.js`。
