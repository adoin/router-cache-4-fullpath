...
The experimental router lives next to the stable one and is opt-in. You import the factory from `vue-smart-router/experimental` and a resolver (from `vue-smart-router/auto-resolver` when using file-based routing):

```ts{3,4,8}
// src/router/index.ts
import { createWebHistory } from 'vue-smart-router'
import { experimental_createRouter as createRouter } from 'vue-smart-router/experimental'
import { resolver, handleHotUpdate } from 'vue-smart-router/auto-resolver'
