...
```ts [rollup.config.js]
import VueRouter from 'vue-x-router/unplugin/rollup'
...
module.exports = {
  /* ... */
  plugins: [
    require('vue-x-router/unplugin/webpack')({
      /* options */
    }),
  ],
}
...
import VueRouter from 'vue-x-router/unplugin/esbuild'
...
/* prettier-ignore */
// @ts-nocheck
// Generated by vue-x-router. ‼️ DO NOT MODIFY THIS FILE ‼️
...
declare module 'vue-x-router/auto-routes' {
  ...
  /**
   * Route name map generated by vue-x-router file-based routing
   */
...
```jsonc [tsconfig.json]
{
  "vueCompilerOptions": {
    "plugins": [
      "vue-x-router/volar/sfc-route-blocks",
      "vue-x-router/volar/sfc-typed-router",
    ],
  },
}
```

- `vue-x-router/volar/sfc-route-blocks` — enables `<route>` blocks in SFCs for defining per-page route metadata
- `vue-x-router/volar/sfc-typed-router` — makes `useRoute()` return a typed route based on the current page component, so `route.params` is correctly typed
...
import { routes, handleHotUpdate } from 'vue-x-router/auto-routes' // [!code ++]
...
- Import the `routes` from `vue-x-router/auto-routes` and pass them to the `createRouter` function.
...
import { routes } from 'vue-x-router/auto-routes' // [!code ++]
